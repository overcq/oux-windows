...
unsigned long
E_main_Q_w0_R_strlen( wchar_t *s
){  unsigned long ret = 0;
    while( *s )
    {   ret++;
        s++;
...
// Wide-string helpers
int
E_main_Q_w0_w0_I_strcmp( wchar_t *s
, wchar_t *t
){  while( *s
    && *t
    && *s == *t
...
wchar_t *
E_main_Q_w_w0_I_strcpy( wchar_t *s
, wchar_t *t
){  wchar_t *s_ = s;
    do
    {   *s_ = *t;
...
char *
E_main_Q_w0_I_to_s0( wchar_t *w
){  if( !w ) return 0;
    int needed = WideCharToMultiByte( CP_UTF8, 0, w, -1, 0, 0, 0, 0 );
    if( needed <= 0 )
        return 0;
    char *ret = HeapAlloc( E_main_S_process_heap, 0, needed );
    if( !ret )
        return 0;
    int r = WideCharToMultiByte( CP_UTF8, 0, w, -1, ret, needed, 0, 0 );
    if( r == 0 )
...
// Wide-buffer memory wrappers (handle sizeof(wchar_t) internally)
void *
wHeapAlloc( HANDLE hHeap, DWORD dwFlags, size_t nElements
){  return HeapAlloc( hHeap, dwFlags, nElements * sizeof( wchar_t ));
}
void *
wHeapReAlloc( HANDLE hHeap, DWORD dwFlags, void *lpMem, size_t nElements
){  return HeapReAlloc( hHeap, dwFlags, lpMem, nElements * sizeof( wchar_t ));
}
void
wCopyMemory( void *dest, const void *src, size_t nElements
){  CopyMemory( dest, src, nElements * sizeof( wchar_t ));
}
void
wZeroMemory( void *mem, size_t nElements
){  ZeroMemory( mem, nElements * sizeof( wchar_t ));
}
//==============================================================================
#define V(value,msg) if( (value) || ( !E_main_I_check_winapi_error_internal( __LINE__, msg), FALSE )){}else
#define Vv(value,msg) if( (value) || ( !E_main_I_check_winapi_error_v_internal( __LINE__, msg, #value, ( unsigned long long )value ), FALSE )){}else
int
E_main_I_check_winapi_error_internal( int line
, char *msg
){  DWORD e = GetLastError();
    int len = MultiByteToWideChar( CP_UTF8, MB_ERR_INVALID_CHARS, msg, -1, 0, 0 );
    if( len <= 0 )
    {   fwprintf( stderr, L"\nUnable to convert message: %lu.\n", GetLastError() );
        return ~0;
    }
    wchar_t *msg_w = wHeapAlloc( E_main_S_process_heap, 0, len );
    if( !msg_w )
    {   fwprintf( stderr, L"\nUnable to allocate message: %lu.\n", GetLastError() );
        return ~0;
    }
    MultiByteToWideChar( CP_UTF8, MB_ERR_INVALID_CHARS, msg, -1, msg_w, len );
    fwprintf( stderr, L"In line %d: %ls", line, msg_w );
    HeapFree( E_main_S_process_heap, 0, msg_w );
    wchar_t *s;
...
, unsigned long long value
    ...
    MultiByteToWideChar( CP_UTF8, MB_ERR_INVALID_CHARS, msg, -1, msg_w, len );
    wchar_t *name_w = 0;
    int nlen = MultiByteToWideChar( CP_UTF8, MB_ERR_INVALID_CHARS, name, -1, 0, 0 );
    if( nlen > 0 )
    {   name_w = wHeapAlloc( E_main_S_process_heap, 0, nlen );
        if( name_w )
            MultiByteToWideChar( CP_UTF8, MB_ERR_INVALID_CHARS, name, -1, name_w, nlen );
    }
    fwprintf( stderr, L"In line %d: %ls: %ls = %llu", line, msg_w, name_w ? name_w : L"(null)", value );
    if( name_w )
        HeapFree( E_main_S_process_heap, 0, name_w );
    HeapFree( E_main_S_process_heap, 0, msg_w );
    wchar_t *s;
    if( !FormatMessage( FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_FROM_SYSTEM, 0, e, MAKELANGID( LANG_NEUTRAL, SUBLANG_DEFAULT ), ( LPTSTR )&s, 0, 0 ))
    {   fwprintf( stderr, L"\nUnable to format message: %lu.\n", GetLastError() );
...
E_main_Q_build_resources_M( struct E_main_Z_build_resources *data
    ...
    Vv( data->file_h_to_libs_buffer, "Unable to allocate" )
        return 1;
    CopyMemory( data->file_h_to_libs_buffer, "#ifndef UNICODE\r\n#define UNICODE\r\n#endif\r\n#include <windows.h>\r\n#include \"E_cplus_S_machine.h\"\r\n#include \"E_cplus_S_language.h\"\r\n#include \"E_cplus_S_simple.h\"\r\n#include \"E_cplus_S_base.h\"\r\n", data->file_h_to_libs_buffer_size );
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
char *
E_main_Q_filename_I_encode( wchar_t *filename
){  char *filename_utf = E_main_Q_w0_I_to_s0(filename);
    if( !filename_utf )
        return 0;
    unsigned long size = E_main_Q_s0_R_strlen( filename_utf ) - 3 + 1;
    char *ret = HeapAlloc( E_main_S_process_heap, 0, size );
    Vv( ret, "Unable to allocate" )
    {   HeapFree( E_main_S_process_heap, 0, ret );
        HeapFree( E_main_S_process_heap, 0, filename_utf );
        return 0;
    }
    CopyMemory( ret, filename_utf, size - 1 );
    ret[ size - 1 ] = '\0';
    HeapFree( E_main_S_process_heap, 0, filename_utf );
    for( char *p = ret; *p; p++ )
    {   if( E_main_Q_c_I_isalnum( *p ))
        ...
        size += l_added;
        char c = p[ 1 + l_added ];
        sprintf( p, "_%.2x", ( unsigned char )*p );
        p[ 1 + l_added ] = c;
        p += l_added;
    }
    return ret;
...
, int module
    ...
    {   char *s_1 = "#undef DLLIMPORT\r\n#ifdef E_cplus_C_building_S_";
        char *s_2 = "\r\n#define DLLIMPORT __declspec(dllexport)\r\n#else\r\n#define DLLIMPORT __declspec(dllimport)\r\n#endif\r\n";
        char *enc = E_main_Q_w0_I_to_s0( data->found_dir );
        if( !enc )
            return 1;
        unsigned long l = E_main_Q_s0_R_strlen(enc);
        char *s = HeapAlloc( E_main_S_process_heap, 0, 46 + l + 99 + 1 );
        ...
        CopyMemory( s + 46, enc, l );
        HeapFree( E_main_S_process_heap, 0, enc );
        CopyMemory( s + 46 + l, s_2, 99 + 1 );
        unsigned long l_written;
        V( WriteFile( h_file, s, 46 + l + 99, &l_written, 0 )
        && l_written == 46 + l + 99
        , "Unable to write to file"
        )
        {   HeapFree( E_main_S_process_heap, 0, s );
    ...
    {   eof = E_main_Q_file_R_line( h_file_cx, &line );
        ...
        {   int match_res = pcre2_match( data->def_name_regex, ( PCRE2_SPTR8 )line, PCRE2_ZERO_TERMINATED, 0, 0, data->_3_match_data, 0 );
            ...
            {   PCRE2_SIZE *ovector = pcre2_get_ovector_pointer( data->_3_match_data );
                ...
                {   char *s_ = HeapReAlloc( E_main_S_process_heap, 0, s, 1 + 16 + ( match_res == 1 ? 1 : 0 ) + l - 1 + 1 + 1 );
                    ...
                    s = s_;
                    unsigned long l_ = 6 + ( module ? 10 : 0 );
                    MoveMemory( s + 1 + l_ + ( match_res == 1 ? 1 : 0 ), s + 1, l - 1 );
                    if(module)
                        CopyMemory( s + 1, "extern DLLIMPORT", l_ );
                    else
                        CopyMemory( s + 1, "extern", l_ );
                    if( match_res == 1 )
                        s[ 1 + l_ ] = ' ';
                    E_main_Q_s_s0_I_strcpy( s + 1 + l_ + ( match_res == 1 ? 1 : 0 ) + l - 1, ";" );
                }
        ...
        {   if( match_res != PCRE2_ERROR_NOMATCH )
            ...
            {   PCRE2_SIZE *ovector = pcre2_get_ovector_pointer( data->_3_match_data );
                ...
                unsigned long l_ = 7 + ( module ? 10 : 0 );
                char *s = HeapAlloc( E_main_S_process_heap, 0, l_ + l + 1 + 1 );
                Vv( s, "Unable to allocate" )
                    goto Err_2;
                if(module)
                ...
                CopyMemory( s + l_, line, l );
                E_main_Q_s_s0_I_strcpy( s + l_ + l, ";" );
                if( E_main_Q_file_P_line( h_file, s ))
                {   HeapFree( E_main_S_process_heap, 0, s );
                    goto Err_2;
                }
...
int
E_main_I_find_file( wchar_t *file_mask
...
, void *context
    ...
    {   do
        ...
        fwprintf( stderr, L"Not found file mask \"%ls\".\n", file_mask );
    return ret;
}
//------------------------------------------------------------------------------
int
...
, struct E_main_Z_build_resources *data
    ...
    {   unsigned long l_dir = E_main_Q_w0_R_strlen( data->found_dir );
        unsigned long l_file = E_main_Q_w0_R_strlen( found_file->cFileName );
        file = wHeapAlloc( E_main_S_process_heap, 0, 10 + l_dir + 1 + 10 + l_file );
        Vv( file, "Unable to allocate" )
            return 1;
        E_main_Q_w_w0_I_strcpy( file, L"..\\module\\" );
        E_main_Q_w_w0_I_strcpy( file + 10, data->found_dir );
        file[ 10 + l_dir ] = L'\\';
        E_main_Q_w_w0_I_strcpy( file + 10 + l_dir + 1, found_file->cFileName );
        fwprintf( stderr, L"Processing file \"%ls\"...\n", file );
        ...
        V( h_file_cx != INVALID_HANDLE_VALUE, "Unable to open file" )
            goto Err_1;
        E_main_Q_w_w0_I_strcpy( file + 10 + l_dir + 1, L"E_cplus_S_" );
        wCopyMemory( file + 10 + l_dir + 1 + 10, found_file->cFileName, l_file - 1 );
        file[ 10 + l_dir + 1 + 10 + l_file - 1 ] = L'\0';
...
/*        unsigned long l_added = 22 + l_dir + 2 + l_file - 2 + 4;
        ...
        wchar_t *path = wHeapAlloc( E_main_S_process_heap, 0, l_dir + 1 + l_file + 1 );
        Vv( path, "Unable to allocate" )
            goto Err_2;
        E_main_Q_w_w0_I_strcpy( path, data->found_dir );
        path[ l_dir ] = L'\\';
        E_main_Q_w_w0_I_strcpy( path + l_dir + 1, found_file->cFileName );
        ...
        char *enc_dir = E_main_Q_w0_I_to_s0( data->found_dir );
        Vv( enc_dir, "Unable to allocate" )
            return 1;
        E_main_Q_s_s0_I_strcpy( s + 22, enc_dir );
        E_main_Q_s_s0_I_strcpy( s + 22 + l_dir, "__" );
        char *found_file_s = E_main_Q_w0_I_to_s0( found_file->cFileName );
        Vv( found_file_s, "Unable to allocate" )
        {   HeapFree( E_main_S_process_heap, 0, enc_dir );
            return 1;
        }
        CopyMemory( s + 22 + l_dir + 2, found_file_s, l_file - 2 );
        ...
        file_h_to_libs_buffer_ = HeapReAlloc( E_main_S_process_heap, 0, data->file_h_to_libs_buffer, data->file_h_to_libs_buffer_size + l_added );
        Vv( file_h_to_libs_buffer_, "Unable to reallocate" )
        {   HeapFree( E_main_S_process_heap, 0, found_file_s );
            goto Err_2;
        }
        s = file_h_to_libs_buffer_ + ( s - data->file_h_to_libs_buffer );
        data->file_h_to_libs_buffer = file_h_to_libs_buffer_;
...
, struct E_main_Z_build_resources *data
    ...
    if(( found_file->dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY )
    && found_file->cFileName[0] != L'.'
    )
    {   unsigned long l_dir = E_main_Q_w0_R_strlen( found_file->cFileName );
        ...
        char *enc = E_main_Q_w0_I_to_s0( found_file->cFileName );
        Vv( enc, "Unable to allocate" )
            return 1;
        E_main_Q_s_s0_I_strcpy( s + 10, enc );
        CopyMemory( s + 10 + l_dir, ".h\"\r\n", 5 );
        HeapFree( E_main_S_process_heap, 0, enc );
        data->found_dir = found_file->cFileName;
        wchar_t *file_mask = wHeapAlloc( E_main_S_process_heap, 0, 10 + l_dir + 5 + 1 );
        Vv( file_mask, "Unable to allocate" )
            return 1;
        E_main_Q_w_w0_I_strcpy( file_mask, L"..\\module\\" );
        E_main_Q_w_w0_I_strcpy( file_mask + 10, found_file->cFileName );
        E_main_Q_w_w0_I_strcpy( file_mask + 10 + l_dir, L"\\*.cx" );
...
, struct E_main_Z_build_resources *data
    ...
    {   unsigned long l_dir = E_main_Q_w0_R_strlen( data->found_dir );
        unsigned long l_file = E_main_Q_w0_R_strlen( found_file->cFileName );
        file = wHeapAlloc( E_main_S_process_heap, 0, 10 + l_dir + 1 + l_file + 1 );
        Vv( file, "Unable to allocate" )
            return 1;
        E_main_Q_w_w0_I_strcpy( file, L"..\\module\\" );
        E_main_Q_w_w0_I_strcpy( file + 10, data->found_dir );
        ...
        file_h = wHeapAlloc( E_main_S_process_heap, 0, 10 + l_dir + 1 + 12 + l_dir + 2 + l_file - 2 + 1 + 1 );
        Vv( file_h, "Unable to allocate" )
            goto Err_1;
        wCopyMemory( file_h, file, 10 + l_dir + 1 );
        E_main_Q_w_w0_I_strcpy( file_h + 10 + l_dir + 1, L"E_cplus_S_1_" );
        E_main_Q_w_w0_I_strcpy( file_h + 10 + l_dir + 1 + 12, data->found_dir );
        E_main_Q_w_w0_I_strcpy( file_h + 10 + l_dir + 1 + 12 + l_dir, L"__" );
        wCopyMemory( file_h + 10 + l_dir + 1 + 12 + l_dir + 2, found_file->cFileName, l_file - 2 );
        E_main_Q_w_w0_I_strcpy( file_h + 10 + l_dir + 1 + 12 + l_dir + 2 + l_file - 2, L"h" );
...
, struct E_main_Z_build_resources *data
    ...
    {   unsigned long l_dir = E_main_Q_w0_R_strlen( found_file->cFileName );
        data->found_dir = wHeapAlloc( E_main_S_process_heap, 0, l_dir + 1 );
        Vv( data->found_dir, "Unable to allocate" )
            return 1;
        E_main_Q_w_w0_I_strcpy( data->found_dir, found_file->cFileName );
        wchar_t *file_mask = wHeapAlloc( E_main_S_process_heap, 0, 10 + l_dir + 5 + 1 );
        Vv( file_mask, "Unable to allocate" )
...
E_main_I_build_I_to_libs( struct E_main_Z_build_resources *data
    ...
    HANDLE h_file_h_to_libs = CreateFile( L"..\\module\\E_cplus_S_to_libs.h", GENERIC_WRITE, FILE_SHARE_READ, 0, CREATE_ALWAYS, 0, 0 );
...
#endif
    unsigned long l_file_mask = E_main_Q_w0_R_strlen( data->found_dir );
    wchar_t *file_h_not_to_libs = wHeapAlloc( E_main_S_process_heap, 0, l_file_mask - 1 + 23 + 1 );
    V( file_h_not_to_libs, "Unable to allocate" )
        return 1;
    wCopyMemory( file_h_not_to_libs, data->found_dir, l_file_mask - 1 );
    E_main_Q_w_w0_I_strcpy( file_h_not_to_libs + l_file_mask - 1, L"E_cplus_S_not_to_libs.h" );
...
, struct E_main_Z_build_resources *data
    ...
    {   unsigned long l_file_mask = E_main_Q_w0_R_strlen( data->found_dir );
        ...
        {   unsigned long file_h_not_to_libs_buffer_size = data->file_h_not_to_libs_buffer_size;
            ...
            unsigned long l_dir = E_main_Q_w0_R_strlen( found_file->cFileName );
            wchar_t *file_mask = wHeapAlloc( E_main_S_process_heap, 0, l_file_mask - 1 + l_dir + 2 + 1 );
            Vv( file_mask, "Unable to allocate" )
                return 1;
            wCopyMemory( file_mask, data->found_dir, l_file_mask - 1 );
            E_main_Q_w_w0_I_strcpy( file_mask + l_file_mask - 1, found_file->cFileName );
            E_main_Q_w_w0_I_strcpy( file_mask + l_file_mask - 1 + l_dir, L"\\*" );
            wchar_t *found_dir = data->found_dir;
        ...
        {   unsigned long l_file = E_main_Q_w0_R_strlen( found_file->cFileName );
            if( l_file > 3
            && !E_main_Q_w0_w0_I_strcmp( found_file->cFileName + l_file - 3, L".cx" )
            )
            {   file = wHeapAlloc( E_main_S_process_heap, 0, l_file_mask - 1 + 12 + l_file + 1 );
                Vv( file, "Unable to allocate" )
                    return 1;
                wCopyMemory( file, data->found_dir, l_file_mask - 1 );
                E_main_Q_w_w0_I_strcpy( file + l_file_mask - 1, found_file->cFileName );
                fwprintf( stderr, L"Processing file \"%ls\"...\n", file );
                h_file_cx = CreateFile( file, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, 0 );
                V( h_file_cx != INVALID_HANDLE_VALUE, "Unable to open file" )
                    goto Err_1;
                E_main_Q_w_w0_I_strcpy( file + l_file_mask - 1, L"E_cplus_S_" );
                wCopyMemory( file + l_file_mask - 1 + 10, found_file->cFileName, l_file - 1 );
                file[ l_file_mask - 1 + 10 + l_file - 1 ] = L'\0';
                h_file = CreateFile( file, GENERIC_WRITE, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0 );
                if( h_file != INVALID_HANDLE_VALUE )
                {   FILETIME file_time;
                    if( GetFileTime( h_file, 0, 0, &file_time ))
...
Cont_1:         CloseHandle( h_file );
                ...
                data->file_h_not_to_libs_buffer_size += l_added;
                E_main_Q_s_s0_I_strcpy( s, "#include \"E_cplus_S_1_" );
                char *enc_file = E_main_Q_w0_I_to_s0( found_file->cFileName );
                Vv( enc_file, "Unable to allocate" )
                    goto Err_2;
                l_enc = E_main_Q_s0_R_strlen( enc_file );
                CopyMemory( s + 22, enc_file, l_enc - 2 );
                CopyMemory( s + 22 + l_enc - 2, "h\"\r\n", 4 );
                s += 22 + l_enc - 2 + 4;
                do
                {   s = E_main_Q_s0_I_strstr( s, "\r\n" ) + 2;
                    if( s - data->file_h_not_to_libs_buffer + 22 > data->file_h_not_to_libs_buffer_size
                ...
                file_h_not_to_libs_buffer_ = HeapReAlloc( E_main_S_process_heap, 0, data->file_h_not_to_libs_buffer, data->file_h_not_to_libs_buffer_size + l_added );
                Vv( file_h_not_to_libs_buffer_, "Unable to reallocate" )
                {   HeapFree( E_main_S_process_heap, 0, enc_file );
                    goto Err_3;
                }
                s = file_h_not_to_libs_buffer_ + ( s - data->file_h_not_to_libs_buffer );
                data->file_h_not_to_libs_buffer = file_h_not_to_libs_buffer_;
                ...
                HeapFree( E_main_S_process_heap, 0, enc_file );
                CopyMemory( s + 22 + l_enc - 2, "h\"\r\n", 4 );
                E_main_Q_w_w0_I_strcpy( file + l_file_mask - 1, L"E_cplus_S_1_" );
                wCopyMemory( file + l_file_mask - 1 + 12, found_file->cFileName, l_file - 2 );
                E_main_Q_w_w0_I_strcpy( file + l_file_mask - 1 + 12 + l_file - 2, L"h" );
                h_file = CreateFile( file, GENERIC_WRITE, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0 );
                if( h_file != INVALID_HANDLE_VALUE )
                {   FILETIME file_time;
                    if( GetFileTime( h_file, 0, 0, &file_time ))
...
, struct E_main_Z_build_resources *data
    ...
    {   unsigned long l_dir = E_main_Q_w0_R_strlen( found_file->cFileName );
        wchar_t *file_h_not_to_libs = wHeapAlloc( E_main_S_process_heap, 0, 11 + l_dir + 24 + 1 );
        Vv( file_h_not_to_libs, "Unable to allocate" )
            return 1;
        E_main_Q_w_w0_I_strcpy( file_h_not_to_libs, L"..\\program\\" );
        E_main_Q_w_w0_I_strcpy( file_h_not_to_libs + 11, found_file->cFileName );
        E_main_Q_w_w0_I_strcpy( file_h_not_to_libs + 11 + l_dir, L"\\E_cplus_S_not_to_libs.h" );
        ...
        Vv( data->file_h_not_to_libs_buffer, "Unable to allocate" )
            return 1;
        CopyMemory( data->file_h_not_to_libs_buffer, data->file_h_to_libs_buffer, data->file_h_to_libs_buffer_size );
        wchar_t *file_mask = wHeapAlloc( E_main_S_process_heap, 0, 11 + l_dir + 2 + 1 );
        E_main_Q_w_w0_I_strcpy( file_mask, L"..\\program\\" );
        E_main_Q_w_w0_I_strcpy( file_mask + 11, found_file->cFileName );
        E_main_Q_w_w0_I_strcpy( file_mask + 11 + l_dir, L"\\*" );
        data->found_dir = file_mask;
        !( ret = E_main_I_find_file( file_mask, ( E_main_I_find_file_Z_proc )E_main_I_find_file_I_program, data ))
        && !( ret = E_main_I_build_program_I_not_to_libs(data) );
        HeapFree( E_main_S_process_heap, 0, file_mask );
...
E_main_I_build( void
    ...
    if(ret)
        return 1;
    !( ret = E_main_I_find_file( L"..\\module\\*", ( E_main_I_find_file_Z_proc )E_main_I_find_file_I_modules_1, &data ))
    && !( ret = E_main_I_find_file( L"..\\module\\*", ( E_main_I_find_file_Z_proc )E_main_I_find_file_I_modules_2, &data ))
    && !( ret = E_main_I_build_I_to_libs( &data ))
    && !( ret = E_main_I_find_file( L"..\\program\\*", ( E_main_I_find_file_Z_proc )E_main_I_find_file_I_programs, &data ));
...
, struct E_main_Z_build_resources *data
    ...
    {   unsigned long l_dir = E_main_Q_w0_R_strlen( data->found_dir );
        unsigned long l_file = E_main_Q_w0_R_strlen( found_file->cFileName );
        wchar_t *file = wHeapAlloc( E_main_S_process_heap, 0, 10 + l_dir + 1 + 12 + l_dir + 2 + l_file - 1 + 1 );
        Vv( file, "Unable to allocate" )
            return 1;
        E_main_Q_w_w0_I_strcpy( file, L"..\\module\\" );
        E_main_Q_w_w0_I_strcpy( file + 10, data->found_dir );
        ...
        E_main_Q_w_w0_I_strcpy( file + 10 + l_dir + 1, L"E_cplus_S_" );
        wCopyMemory( file + 10 + l_dir + 1 + 10, found_file->cFileName, l_file - 1 );
        E_main_Q_w_w0_I_strcpy( file + 10 + l_dir + 1 + 10 + l_file - 2, L"o" );
        fwprintf( stderr, L"Removing file \"%ls\"...\n", file );
        DeleteFile(file);
        E_main_Q_w_w0_I_strcpy( file + 10 + l_dir + 1 + 10, L"1_" );
        E_main_Q_w_w0_I_strcpy( file + 10 + l_dir + 1 + 12, data->found_dir );
        E_main_Q_w_w0_I_strcpy( file + 10 + l_dir + 1 + 12 + l_dir, L"__" );
        wCopyMemory( file + 10 + l_dir + 1 + 12 + l_dir + 2, found_file->cFileName, l_file - 2 );
        E_main_Q_w_w0_I_strcpy( file + 10 + l_dir + 1 + 12 + l_dir + 2 + l_file - 2, L"h" );
        fwprintf( stderr, L"Removing file \"%ls\"...\n", file );
        DeleteFile(file);
        file[ 10 + l_dir + 1 + 10 ] = L'2';
        fwprintf( stderr, L"Removing file \"%ls\"...\n", file );
        DeleteFile(file);
        wCopyMemory( file + 10 + l_dir + 1 + 10, found_file->cFileName, l_file - 1 );
        file[ 10 + l_dir + 1 + 10 + l_file - 1 ] = L'\0';
...
, struct E_main_Z_build_resources *data
    ...
    {   unsigned long l_file_mask = E_main_Q_w0_R_strlen( data->found_dir );
        if( found_file->dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY )
        {   wchar_t *file = wHeapAlloc( E_main_S_process_heap, 0, l_file_mask - 1 + 23 + 1 );
            Vv( file, "Unable to allocate" )
                return 1;
            wCopyMemory( file, data->found_dir, l_file_mask - 1 );
            E_main_Q_w_w0_I_strcpy( file + l_file_mask - 1, L"E_cplus_S_not_to_libs.h" );
            char *enc_file = E_main_Q_w0_I_to_s0(file);
            Vv( enc_file, "Unable to allocate" )
                return 1;
            HeapFree( E_main_S_process_heap, 0, enc_file );
            fwprintf( stderr, L"Removing file \"%ls\"...\n", file );
        ...
        {   unsigned long l_file = E_main_Q_w0_R_strlen( found_file->cFileName );
            ...
            {   wchar_t *file = wHeapAlloc( E_main_S_process_heap, 0, l_file_mask - 1 + 12 + l_file );
                Vv( file, "Unable to allocate" )
                    return 1;
                wCopyMemory( file, data->found_dir, l_file_mask - 1 );
                E_main_Q_w_w0_I_strcpy( file + l_file_mask - 1, L"E_cplus_S_" );
                wCopyMemory( file + l_file_mask - 1 + 10, found_file->cFileName, l_file - 2 );
                E_main_Q_w_w0_I_strcpy( file + l_file_mask - 1 + 10 + l_file - 2, L"o" );
                char *enc_file = E_main_Q_w0_I_to_s0(file);
                Vv( enc_file, "Unable to allocate" )
                    return 1;
                HeapFree( E_main_S_process_heap, 0, enc_file );
                fwprintf( stderr, L"Removing file \"%ls\"...\n", file );
                E_main_Q_w_w0_I_strcpy( file + l_file_mask - 1 + 10, L"1_" );
                wCopyMemory( file + l_file_mask - 1 + 12, found_file->cFileName, l_file - 2 );
                E_main_Q_w_w0_I_strcpy( file + l_file_mask - 1 + 12 + l_file - 2, L"h" );
                enc_file = E_main_Q_w0_I_to_s0(file);
                Vv( enc_file, "Unable to allocate" )
                    return 1;
                HeapFree( E_main_S_process_heap, 0, enc_file );
                fwprintf( stderr, L"Removing file \"%ls\"...\n", file );
...
, struct E_main_Z_build_resources *data
    ...
    {   unsigned long l_dir = E_main_Q_w0_R_strlen( found_file->cFileName );
        wchar_t *file = wHeapAlloc( E_main_S_process_heap, 0, 11 + l_dir + 24 + 1 );
        Vv( file, "Unable to allocate" )
            return 1;
        E_main_Q_w_w0_I_strcpy( file, L"..\\program\\" );
        E_main_Q_w_w0_I_strcpy( file + 11, found_file->cFileName );
        E_main_Q_w_w0_I_strcpy( file + 11 + l_dir, L"\\E_cplus_S_not_to_libs.h" );
        fwprintf( stdout, L"Removing file \"%ls\"...\n", file );
        DeleteFile(file);
        wchar_t *file_mask = wHeapReAlloc( E_main_S_process_heap, 0, file, 11 + l_dir + 1 + 1 + 1 );
        Vv( file_mask, "Unable to reallocate" )
        {   HeapFree( E_main_S_process_heap, 0, file );
            return 1;
        }
        data->found_dir = file_mask;
        E_main_Q_w_w0_I_strcpy( file_mask + 11 + l_dir + 1, L"*" );
        ret = E_main_I_find_file( file_mask, ( E_main_I_find_file_Z_proc )E_main_I_find_file_I_clean_program, data );
        HeapFree( E_main_S_process_heap, 0, file_mask );
    }
    return ret;
...
E_main_I_clean( void
    ...
    DeleteFile( L"..\\module\\E_cplus_S_to_libs.h" );
    struct E_main_Z_build_resources data;
    int ret = 0;
    !( ret = E_main_I_find_file( L"..\\module\\*", ( E_main_I_find_file_Z_proc )E_main_I_find_file_I_clean_modules, &data ))
    && !( ret = E_main_I_find_file( L"..\\program\\*", ( E_main_I_find_file_Z_proc )E_main_I_find_file_I_clean_programs, &data ));
    return ret;
}
//==============================================================================
enum E_main_Z_func
